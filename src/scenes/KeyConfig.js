...
const KeyConfigScene = function KeyConfigSceneFunc() {
    const state = new Phaser.Scene(gameConfig.SCENES.KEY_CONFIG);
    let titleText;
    ...
    function _onKeyDown(e) {
        if (e.keyCode === gameConfig.KEYS.ESCAPE.CODE) {
            const data = {
                sourceScene: gameConfig.SCENES.KEY_CONFIG,
            };
    ...
    function create() {
        if (!titleText) {
            titleText = state.add.text(gameConfig.GAME.VIEWWIDTH / 2, 20, 'Key Config - TODO', {
                font: '72px Arial',
                fill: '#eeeeee',
                align: 'center',
                stroke: '#000000',
